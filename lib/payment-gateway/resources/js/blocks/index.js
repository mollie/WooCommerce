...
window.inpsydeGateways && inpsydeGateways.forEach((name) => {
    ...
    const Content = (props) => {
        ...
        return (
            <>{components.map((Component, index) => <Component key={index} {...props} />)}</>
        );
    };
    /**
     * Label component
    ...
    const Label = (props) => {
        ...
        const icons = defaultHooks.applyFilters(iconsHookName, settings.icons);
        return <>
            <PaymentMethodLabel text={label} />
            {PaymentMethodIcons && icons && Array.isArray(icons) && icons.length > 0 && (
                <>
                    {icons.map((icon, index) => (
                        <PaymentMethodIcons key={index} icons={[icon]} />
                    ))}
                </>
            )}
        </>;
    ...
    const SavedTokenContent = (props) => {
        ...
        return (
            <>{components.map((Component, i) => <Component key={i} {...props} />)}</>
        );
    };
