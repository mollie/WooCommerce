...
class AdminNotice implements NoticeInterface
{
    public function addNotice($level, $message): void
    {
        add_action(
            ...
            function () use ($level, $message) {
                echo $this->renderNotice((string) $level, (string) $message); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- escaped in renderNotice()
            }
        );
    }
    ...
    public function renderNotice(string $level, string $message): string
    {
        ...
        $errorAttribute = in_array('notice-error', $classes, true) ? ' data-mollie-error' : '';

        return '<div class="notice ' . esc_attr($level) . '"' . $errorAttribute . ' style="padding:12px 12px">'
