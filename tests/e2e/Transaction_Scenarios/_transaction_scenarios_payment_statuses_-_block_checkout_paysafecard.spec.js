...
test.describe('_Transaction scenarios_Payment statuses - Block Checkout - paysafecard', () => {
    ...
    testData.forEach(({ testId, mollieStatus, wooStatus, notice, action }) => {
        test(`[TestId-${testId}] Validate the submission of an order with paysafecard as payment method and payment mark as "${mollieStatus} on block checkout"`, async ({ page, products, context }) => {
