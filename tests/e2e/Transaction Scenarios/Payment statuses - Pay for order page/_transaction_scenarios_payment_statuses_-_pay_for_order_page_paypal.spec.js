...
test.describe('_Transaction scenarios_Payment statuses - Pay for order page - PayPal', () => {
    ...
    testData.forEach(({ testId, mollieStatus, wooStatus, notice, action }) => {
        test(`[${testId}] Validate the submission of an order with Paypal as payment method and payment mark as "${mollieStatus} on pay for order page"`, async ({ page, products, context }) => {
