			...
			const updateSurcharge = () => {
				jQuery.ajax( {
					...
					success: ( response ) => {
						...
						if ( result ) {
							const productTotal =
								"</th><td class='product-total'><span class='woocommerce-Price-amount amount'><bdi><span class='woocommerce-Price-currencySymbol'>" +
								result.currency +
								'</span>' +
								result.newTotal +
								'</bdi></span></td>';
							...
							if ( ! result.amount ) {
								if (
									$(
										'#order_review table:first-child tfoot tr'
									)
										.text()
										.indexOf( gatewayLabel ) !== -1
								) {
									$(
										'#order_review table:first-child tfoot tr:contains("' +
											gatewayLabel +
											'")'
									).remove();
									$(
										'#order_review table:first-child tfoot tr:last td'
									).replaceWith( productTotal );
								}
							} else {
								const tableRow =
								...
								if (
									...
									$(
										'#order_review table:first-child tfoot tr:first'
									).after( tableRow );
									$(
										'#order_review table:first-child tfoot tr:last td'
									).replaceWith( productTotal );
