...
final class SessionPayload
{
    ...
    {
        $total = $cart->total();
        return ['amount' => $total === null ? null : ['currency' => $total->currency(), 'value' => $total->toDecimal()], 'description' => self::DESCRIPTION, 'lines' => $lines, 'redirectUrl' => $redirectUrl, 'payment' => ['webhookUrl' => $webhookUrl], 'metadata' => $metadata, 'requiredCustomerDetails' => array_values($requiredCustomerDetails)];
    }
    /**
