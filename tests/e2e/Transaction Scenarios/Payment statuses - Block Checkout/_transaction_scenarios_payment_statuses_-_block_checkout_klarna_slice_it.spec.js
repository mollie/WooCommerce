...
test.describe('_Transaction scenarios_Payment statuses - Block Checkout - Klarna Slice it', () => {
    ...
    testData.forEach(({ testId, mollieStatus, wooStatus, notice, action }) => {
        test(`[${testId}] Validate the submission of an order with Klarna Slice it as payment method and payment mark as "${mollieStatus} on block checkout"`, async ({ page, products, context }) => {
