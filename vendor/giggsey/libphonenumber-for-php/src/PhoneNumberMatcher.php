...
class PhoneNumberMatcher implements \Iterator
{
    ...
    protected static function init()
    {
        ...
        static::$pattern = '(?:' . $leadClass . $punctuation . ')' . $leadLimit . $digitSequence . '(?:' . $punctuation . $digitSequence . ')' . $blockLimit . '(?:' . PhoneNumberUtil::$EXTN_PATTERNS_FOR_MATCHING . ')?';
        ...
        foreach (static::$innerMatches as $possibleInnerMatch) {
            ...
            while ($groupMatcher->find() && $this->maxTries > 0) {
                if ($isFirstMatch) {
                    ...
                    $group = static::trimAfterFirstMatch(PhoneNumberUtil::$UNWANTED_END_CHAR_PATTERN, \mb_substr($candidate, 0, $groupMatcher->start()));
                ...
                $group = static::trimAfterFirstMatch(PhoneNumberUtil::$UNWANTED_END_CHAR_PATTERN, $groupMatcher->group(1));
     ...
     * @return bool
     */
    public static function allNumberGroupsRemainGrouped(PhoneNumberUtil $util, PhoneNumber $number, $normalizedCandidate, $formattedNumberGroups)
    {
        $fromIndex = 0;
     ...
     * @return bool
     */
    public static function allNumberGroupsAreExactlyPresent(PhoneNumberUtil $util, PhoneNumber $number, $normalizedCandidate, $formattedNumberGroups)
    {
     ...
     * @return string[]
     */
    protected static function getNationalNumberGroups(PhoneNumberUtil $util, PhoneNumber $number, ?NumberFormat $formattingPattern = null)
    {
        ...
        $nationalSignificantNumber = $util->getNationalSignificantNumber($number);
        return \explode('-', $util->formatNsnUsingPattern($nationalSignificantNumber, $formattingPattern, PhoneNumberFormat::RFC3966));
    }
    /**
     * @param string $candidate
     * @return bool
     */
    public static function checkNumberGroupingIsValid(PhoneNumber $number, $candidate, PhoneNumberUtil $util, \Closure $checker)
    {
        $normalizedCandidate = PhoneNumberUtil::normalizeDigits($candidate, \true);
    ...
    public static function containsMoreThanOneSlashInNationalNumber(PhoneNumber $number, $candidate)
    {
        ...
        $candidateHasCountryCode = $number->getCountryCodeSource() === CountryCodeSource::FROM_NUMBER_WITH_PLUS_SIGN || $number->getCountryCodeSource() === CountryCodeSource::FROM_NUMBER_WITHOUT_PLUS_SIGN;
        if ($candidateHasCountryCode && PhoneNumberUtil::normalizeDigitsOnly(\mb_substr($candidate, 0, $firstSlashInBodyIndex)) == $number->getCountryCode()) {
