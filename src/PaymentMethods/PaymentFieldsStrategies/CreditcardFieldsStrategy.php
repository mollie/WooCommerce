...
class CreditcardFieldsStrategy extends \Mollie\WooCommerce\PaymentMethods\PaymentFieldsStrategies\AbstractPaymentFieldsRenderer implements PaymentFieldsRendererInterface
{
    public function renderFields(): string
    {
        ...
        $output .= sprintf(esc_html__('%1$s Secure payments provided by %2$s', 'mollie-payments-for-woocommerce'), wp_kses((string) $this->lockIcon($this->dataHelper), $allowedHtml), wp_kses((string) $this->mollieLogo($this->dataHelper), $allowedHtml));
        $output .= '</p>';
        return $output;
    }
    /**
