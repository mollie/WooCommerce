...
function requireField( inputName, fieldId ) {
	jQuery( 'form[name="checkout"] input[name=' + inputName + ']' ).attr(
		'required',
		''
	);
	jQuery( 'form[name="checkout"] p#' + fieldId ).addClass(
		'validate-required'
	);
	jQuery( 'form[name="checkout"] p#' + fieldId + ' label span' ).replaceWith(
...
function unrequireField( inputName, fieldId ) {
	jQuery( 'form[name="checkout"] input[name=' + inputName + ']' ).removeAttr(
		'required'
	);
	jQuery( 'form[name="checkout"] p#' + fieldId ).removeClass(
		'validate-required'
	);
	jQuery( 'form[name="checkout"] p#' + fieldId + ' label abbr' ).replaceWith(
		'<span class="optional">(optional)</span>'
	);
}
function restoreOriginalField(
	billingField,
	positionField,
	fieldMarkup,
	inputName,
	...
	const field = jQuery(
		'form[name="checkout"] input[name=' + inputName + ']'
	);
	const currentVisibility = field.is( ':visible' );
	const currentRequired = field.prop( 'required' );
...
export function maybeRequireField(
	field,
	positionField,
	fieldMarkup,
	inputName,
