...
class Mollie_WC_Plugin
{
	...
	public static function init() {
        ...
        add_filter('woocommerce_payment_gateways', [__CLASS__, 'maybeDisableMealVoucherGateway'], 20);
        add_action(
        ...
        add_filter( 'woocommerce_product_data_panels', [__CLASS__, 'voucherOptionsProductTabContent'] ); // WC 2.6 and up
     ...
     */
    public static function voucherOptionsProductTabContent()
    {
        ...
        woocommerce_wp_select(
                array(
                        ...
                        'type' => 'select',
                        'options' => array(
                                Mollie_WC_Gateway_Mealvoucher::FOOD_AND_DRINKS => 'Food and drinks',
                                Mollie_WC_Gateway_Mealvoucher::HOME_AND_GARDEN => 'Home and garden',
                                Mollie_WC_Gateway_Mealvoucher::GIFTS_AND_FLOWERS => 'Gifts and flowers',
                                Mollie_WC_Gateway_Mealvoucher::NO_CATEGORY => 'No category'
                        ),
                        'default' => Mollie_WC_Gateway_Mealvoucher::FOOD_AND_DRINKS,
    ...
    public static function enqueueApplePayDirectScripts()
    {
        if (mollieWooCommerceIsApplePayDirectEnabled() && is_product()) {
            $applePayHandler = new Mollie_WC_Helper_ApplePayDirectHandler();
            ...
            wp_localize_script(
                ...
                $applePayHandler->applePayScriptData()
            );
        }
    ...
    /**
     * Disable Meal Voucher Gateway
     *
     * @param array $gateways
     * @return array
     */
    public static function maybeDisableMealVoucherGateway(array $gateways)
    {
        ...
        foreach ($products as $product) {
            ...
            $localCategory = $postmeta['_mollie_voucher_category'];
            if(self::productHasVoucherCategory($defaultCategory, $localCategory)){
        ...
        if($productsWithCategory < $numberOfProducts){
            //add notice
            mollieWooCommerceNotice(sprintf(__('Some product in the cart are not enabled to pay via Mealvoucher, select other Payment gateway or remove', 'mollie-payments-for-woocommerce')), 'error');
            //disable send button
            add_filter( 'woocommerce_order_button_html', function(){
                $style = ' style="color:#fff;cursor:not-allowed;background-color:#999;"';
                return '<a class="button alt" ' .$style. ' name="woocommerce_checkout_place_order" id="place_order" >' . __('PLACE ORDER', 'mollie-payments-for-woocommerce') . '</a>';
    ...
    public static function productHasVoucherCategory($defaultCategory, $localCategory)
    {
        if($defaultCategory && $localCategory !== Mollie_WC_Gateway_Mealvoucher::NO_CATEGORY){
            return true;
        }
        if(!$defaultCategory && $localCategory && $localCategory !== Mollie_WC_Gateway_Mealvoucher::NO_CATEGORY){
            return true;
        }
        return false;
    }
