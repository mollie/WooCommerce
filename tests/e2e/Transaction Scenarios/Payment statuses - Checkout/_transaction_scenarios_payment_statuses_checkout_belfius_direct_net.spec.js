...
test.describe('_Transaction scenarios_Payment statuses Checkout - Belfius Direct Net', () => {
    ...
    testData.forEach(({ testId, mollieStatus, wooStatus, notice, action }) => {
        test(`[${testId}] Validate the submission of an order with Belfius Direct Net as payment method and payment mark as "${mollieStatus}"`, async ({ page, products, context }) => {
