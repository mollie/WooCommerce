...
class ExpressOrderFactsBuilder
{
    ...
    public function rememberedSession(): ?RememberedSession
    {
        ...
        return new RememberedSession($remembered['id'], $remembered['ref'], $remembered['fingerprint'], $remembered['expiresAt']);
    }
    /**
    ...
    public function fromOrder(WC_Order $order): ExpressOrderFacts
    {
        ...
        return new ExpressOrderFacts(orderId: $order->get_id(), expressRef: (string) $order->get_meta('_mollie_express_ref'), createdVia: $order->get_created_via(), total: $this->total($order), trackedPaymentId: $tracked !== '' ? $tracked : null, needsPayment: $order->needs_payment(), holdsShipping: $this->holdsShipping($order), needsShipping: $order->needs_shipping_address());
    }
    /**
    ...
    {
        try {
            return \Mollie\WooCommerce\Adapter\WooCommerce\WooCommerceAmount::toMoney($order->get_total('edit'), $order->get_currency());
