...
{
    const ACTION_AFTER_REFUND_AMOUNT_CREATED = 'mollie-payments-for-woocommerce' . '_refund_amount_created';
    const ACTION_AFTER_REFUND_ORDER_CREATED = 'mollie-payments-for-woocommerce' . '_refund_order_created';
    ...
    public function getPaymentObject($paymentId, $testMode = false, $useCache = true)
    {
        try {
            ...
            $this->logger->log(\WC_Log_Levels::DEBUG, __CLASS__ . __FUNCTION__ . ": Could not load payment $paymentId (" . ( $testMode ? 'test' : 'live' ) . "): " . $e->getMessage() . ' (' . get_class($e) . ')');
        }
    ...
    public function getMollieCustomerIbanDetailsFromPaymentObject($payment = null)
    {
        ...
        if (isset($payment->_embedded->payments[0]->id)) {
            $actualPayment = new MolliePayment($payment->_embedded->payments[0]->id, $this->pluginId);
    ...
    public function onWebhookPaid(WC_Order $order, $payment, $paymentMethodTitle)
    {
        ...
        if ($payment->isPaid()) {
            ...
            // Add messages to log
            $this->logger->log(\WC_Log_Levels::DEBUG, __METHOD__ . ' WooCommerce payment_complete() processed and returned to ' . __METHOD__ . " for order {$orderId}");
            ...
            // Add messages to log
            $this->logger->log(\WC_Log_Levels::DEBUG, __METHOD__ . " processing paid order via Mollie plugin fully completed for order {$orderId}");
    ...
    public function onWebhookAuthorized(WC_Order $order, $payment, $paymentMethodTitle)
    {
        ...
        if ($payment->isAuthorized()) {
            ...
            // Add messages to log
            $this->logger->log(\WC_Log_Levels::DEBUG, __METHOD__ . ' processing order status update via Mollie plugin fully completed for order ' . $orderId);

            // Subscription processing
            $this->deleteSubscriptionFromPending($order);
        } else {
            // Add messages to log
            $this->logger->log(\WC_Log_Levels::DEBUG, __METHOD__ . ' order at Mollie not authorized, so no processing for order ' . $orderId);
        }
    }
    ...
    public function onWebhookCompleted(WC_Order $order, $payment, $paymentMethodTitle)
    {
        ...
        if ($payment->isCompleted()) {
            ...
            // Add messages to log
            $this->logger->log(\WC_Log_Levels::DEBUG, __METHOD__ . ' order at Mollie not completed, so no further processing for order ' . $orderId);
        }
    }
    ...
    public function onWebhookFailed(WC_Order $order, $payment, $paymentMethodTitle)
    {
        ...
        $this->logger->log(\WC_Log_Levels::DEBUG, __METHOD__ . ' called for order ' . $orderId . ' and payment ' . $payment->id . ', regular order payment failed.');
    }
    ...
    {
        $this->logger->log(\WC_Log_Levels::DEBUG, __METHOD__ . ' - ' . $orderId . ' - Try to process refunds or cancels.');
        ...
        try {
            ...
            $this->logger->log(\WC_Log_Levels::DEBUG, 'Try to process individual order item refunds or cancels.');
            ...
            try {
                ...
                $this->logger->log(\WC_Log_Levels::DEBUG, __METHOD__ . ' - ' . $exception->getMessage());
            ...
            $this->logger->log(\WC_Log_Levels::DEBUG, __METHOD__ . ' - ' . $exceptionMessage);
        ...
        foreach ($items as $key => $item) {
            ...
            foreach ($paymentObject->lines as $line) {
                ...
                if (empty($line->metadata->order_item_id)) {
                    ...
                    $this->logger->log(\WC_Log_Levels::DEBUG, __METHOD__ . " - " . $noteMessage);
                ...
                if ($originalOrderItemId == $line->metadata->order_item_id) {
                    ...
                    if ((number_format($lineTotalRefundAmount, 2) != number_format($itemRefundAmount, 2)) || ( abs($item->get_quantity()) < 1 )) {
                        ...
                        $this->logger->log(\WC_Log_Levels::DEBUG, __METHOD__ . " - Order $orderId: " . $noteMessage);
                    ...
                    if ($line->status == 'created' || $line->status == 'authorized') {
                        ...
                        $this->logger->log(\WC_Log_Levels::DEBUG, __METHOD__ . ' - Cancelled order line: ' . abs($item->get_quantity()) . 'x ' . $item->get_name() . '. Mollie order line: ' . $line->id . ', payment object: ' . $paymentObject->id . ', order: ' . $orderId . ', amount: ' . $this->data->getOrderCurrency($order) . wc_format_decimal($itemRefundAmount) . ( ! empty($reason) ? ', reason: ' . $reason : '' ));
                    ...
                    if ($line->status == 'paid' || $line->status == 'shipping' || $line->status == 'completed') {
                        ...
                        $this->logger->log(\WC_Log_Levels::DEBUG, __METHOD__ . ' - Refunded order line: ' . abs($item->get_quantity()) . 'x ' . $item->get_name() . '. Mollie order line: ' . $line->id . ', payment object: ' . $paymentObject->id . ', order: ' . $orderId . ', amount: ' . $this->data->getOrderCurrency($order) . wc_format_decimal($itemRefundAmount) . ( ! empty($reason) ? ', reason: ' . $reason : '' ));
                    ...
                    $this->logger->log(\WC_Log_Levels::DEBUG, $noteMessage);
    ...
    public function refund_amount($order, $amount, $paymentObject, $reason)
    {
        ...
        $this->logger->log(\WC_Log_Levels::DEBUG, 'Try to process an amount refund (not individual order line)');
