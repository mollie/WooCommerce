...
class MollieOrderService
{
    ...
    {
        $incomingSecret = isset($_GET['mollie_webhook_secret'])
            ? sanitize_text_field(wp_unslash($_GET['mollie_webhook_secret']))
            : null;
    ...
    public function getRedirectUrlFromPaymentObject($transactionID): string
    {
        ...
        if (!is_string($payment->redirectUrl) || $payment->redirectUrl === '') {
            $this->logger->debug(__METHOD__ . ": payment $transactionID has no redirect URL.", [true]);
            return '';
        }
