...
test.describe('_Transaction scenarios_Payment statuses - Pay for order page - Giropay', () => {
    ...
    testData.forEach(({ testId, mollieStatus, wooStatus, notice, action }) => {
        test(`[TestId-${testId}] Validate the submission of an order with Giropay as payment method and payment mark as "${mollieStatus} on pay for order page"`, async ({ page, products, context }) => {
