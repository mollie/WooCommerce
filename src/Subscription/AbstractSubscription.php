...
abstract class AbstractSubscription extends AbstractGateway
{
	...
	public function update_subscription_status_for_direct_debit( $renewal_order ) {
		...
		if ( wcs_order_contains_renewal( $renewal_order_id ) ) {
			...
			if ( class_exists('WCS_Retry_Manager') && WCS_Retry_Manager::is_retry_enabled() && $subscription->get_date( 'payment_retry' ) > 0 ) {
				Plugin::debug( __METHOD__ . ' - WooCommerce Subscriptions Failed Recurring Payment Retry System in use, not updating subscription status to Active!' );
			...
			try {
				...
				Plugin::debug( 'Could not update subscription ' . $subscription_id . ' status:' . $e->getMessage() );
			}
    ...
    public function scheduled_subscription_payment( $renewal_total, WC_Order $renewal_order )
    {
	    ...
	    if ( ! $renewal_order ) {
		    ...
		    Plugin::debug( $this->id . ': Could not load renewal order or process renewal payment.');
	    ...
	    Plugin::debug($this->id . ': Try to create renewal payment for renewal order ' . $renewal_order_id);
        ...
        try
        {
            ...
            {
                if (isset($mandateId)) {
                    Plugin::debug($this->id . ': Found mandate ID for renewal order ' . $renewal_order_id . ' with customer ID ' . $customer_id );
                    ...
                    Plugin::debug($this->id . ': Try to get all mandates for renewal order ' . $renewal_order_id . ' with customer ID ' . $customer_id );
	        ...
	        try {
		        if ( $validMandate ) {
			        Plugin::debug( $this->id . ': Valid mandate found for renewal order ' . $renewal_order_id );
	        ...
	        Plugin::debug( $this->id . ': Renewal payment ' . $payment->id . ' (' . $payment->mode . ') created for order ' . $renewal_order_id . ' payment json response: ' . json_encode( $payment ) );
	        ...
	        Plugin::debug( $this->id . ': Failed to create payment for order ' . $renewal_order_id . ': ' . $e->getMessage() );
	...
	public function updateFirstPaymentMethodToRecurringPaymentMethod( $renewal_order, $renewal_order_id, $payment ) {
		...
		if ( in_array( $current_method, $methods_needing_update ) && $payment->method == 'directdebit' ) {
            try {
                ...
                Plugin::debug( 'Updating payment method to SEPA Direct Debit failed for renewal order: ' . $renewal_order_id );
            }
		}
	...
	public function restore_mollie_customer_id_and_mandate( $mollie_customer_id, $mollie_payment_id, $subscription ) {
		...
		try {
			...
			if ( empty ( $mollie_customer_id ) ) {
				Plugin::debug( __METHOD__ . ' - Subscription ' . $subscription_id . ' renewal payment: no valid customer ID found, trying to restore from MollieSettingsPage API payment (' . $mollie_payment_id . ').' );
				...
				if ( empty( $mollie_customer_id ) ) {
					Plugin::debug( __METHOD__ . ' - Subscription ' . $subscription_id . ' renewal payment: stopped processing, no customer ID found for this customer/payment combination.' );

					return $mollie_customer_id;
				}

				Plugin::debug( __METHOD__ . ' - Subscription ' . $subscription_id . ' renewal payment: customer ID (' . $mollie_customer_id . ') found, verifying status of customer and mandate(s).' );
			...
			if ( ! $gateway || ! ( $gateway instanceof AbstractGateway ) ) {
				Plugin::debug( __METHOD__ . ' - Subscription ' . $subscription_id . ' renewal payment: stopped processing, not a MollieSettingsPage payment gateway, could not restore customer ID.' );
			...
			if ( $mollie_method == 'creditcard' && ! $mandates->hasValidMandateForMethod( $mollie_method ) ) {
				Plugin::debug( __METHOD__ . ' - Subscription ' . $subscription_id . ' renewal payment: failed! No valid mandate for payment method ' . $mollie_method . ' found.' );
			...
			if ( $mollie_method == 'directdebit' && ! $mandates->hasValidMandateForMethod( $mollie_method ) && $payment_object->isPaid() && $sequence_type == 'oneoff' ) {
				...
				Plugin::debug( __METHOD__ . ' - Subscription ' . $subscription_id . ' renewal payment: no valid mandate for payment method ' . $mollie_method . ' found, trying to create one.' );
				...
				Plugin::debug( __METHOD__ . ' - Subscription ' . $subscription_id . ' renewal payment: mandate created successfully, customer restored.' );

			} else {
				Plugin::debug( __METHOD__ . ' - Subscription ' . $subscription_id . ' renewal payment: the subscription doesn\'t meet the conditions for a mandate restore.' );
			}
			...
			Plugin::debug( __METHOD__ . ' - Subscription ' . $subscription_id . ' renewal payment: customer id and mandate restore failed. ' . $e->getMessage() );
