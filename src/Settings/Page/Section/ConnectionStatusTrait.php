    ...
    protected function connectionErrorMessage(array $connectionStatus): string
    {
        ...
        if ($errorCode >= 500) {
            return sprintf(
                ...
                __('Mollie is currently experiencing issues, please try again later - check the %1$sMollie status page%2$s &#x2716;', 'mollie-payments-for-woocommerce'),
        ...
        if ($errorMessage !== '') {
            return sprintf(
                ...
                __('Communicating with Mollie failed: %1$s &#x2716;', 'mollie-payments-for-woocommerce'),
                esc_html($errorMessage)
            );
        }
        return __('Failed to connect to Mollie API - check your API keys &#x2716;', 'mollie-payments-for-woocommerce');
