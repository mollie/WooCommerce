...
final class FirstSightEffects
{
     ...
     * @return list<Effect>
     */
    public static function for(PaymentSnapshot $payment, ExpressOrderFacts $order, array $wallets, array $registeredGatewayIds): array
    {
        $effects = [Effect::setMeta('_mollie_payment_id', $payment->id()), Effect::setTransactionId($payment->id()), Effect::setMeta('_mollie_payment_mode', $payment->mode())];
        $method = (string) $payment->method();
        $gatewayId = self::gatewayFor($method, $wallets, $registeredGatewayIds);
        $effects[] = $gatewayId !== null ? Effect::setPaymentMethod($gatewayId) : Effect::addNote(self::NOTE_UNKNOWN_WALLET, ['method' => $method]);
    ...
    private static function gatewayFor(string $method, array $wallets, array $registeredGatewayIds): ?string
    {
        foreach ($wallets as $wallet) {
            if ($wallet['mollieMethod'] === $method && in_array($wallet['gatewayId'], $registeredGatewayIds, \true)) {
    ...
    {
        $fields = $address === null ? [] : \Mollie\WooCommerce\Core\Express\AddressMapping::toWooCommerce($address->fields());
