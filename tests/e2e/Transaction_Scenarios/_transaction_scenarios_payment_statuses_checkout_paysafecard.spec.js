...
test.describe('_Transaction scenarios_Payment statuses Checkout - paysafecard', () => {
    ...
    testData.forEach(({ testId, mollieStatus, wooStatus, notice, action }) => {
        test(`[TestId-${testId}] Validate the submission of an order with Paysafecard as payment method and payment mark as "${mollieStatus}"`, async ({ page, products, context }) => {
