...
export const setUpMollieBlockCheckoutListeners = () => {
    ...
    const checkoutStoreCallback = () => {
        try {
            ...
            if (currentPaymentMethod !== paymentMethod) {
                wp.data.dispatch(MOLLIE_STORE_KEY).setActivePaymentMethod(paymentMethod);
    ...
    const unsubscribeCheckoutStore = wp.data.subscribe(checkoutStoreCallback, PAYMENT_STORE_KEY);
