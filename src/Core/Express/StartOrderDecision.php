...
final class StartOrderDecision
{
    ...
    public static function decide(?RememberedSession $session, ?ExpressOrderFacts $existing, CartFacts $cart, int $now): Admit|Refuse
    {
        if ($session === null) {
        ...
        if ($session->fingerprint() !== \Mollie\WooCommerce\Core\Express\PricingFingerprint::of($cart)) {
    ...
    {
        return [Effect::setCreatedVia(self::CREATED_VIA), Effect::setMeta('_mollie_express_ref', $session->expressRef()), Effect::setMeta('_mollie_express_session_id', $session->sessionId()), Effect::setMeta('_mollie_express_expires_at', (string) $session->expiresAt()), Effect::setMeta('_mollie_payment_mode', $mode), Effect::setStatus('pending'), Effect::setPaymentMethod($gatewayId), Effect::addNote(self::NOTE, ['wallet' => $wallet])];
    }
}
