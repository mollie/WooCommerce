...
class WebhookHandler
{
    ...
    public function onWebhookPaid(
        ...
        $this->logger->debug(
            __METHOD__ . " processing paid order via Mollie plugin fully completed for order {$orderId}"
        );
     ...
     * @param MollieObject $mollieObject
     */
    public function onWebhookAuthorized(
        WC_Order $order,
        $payment,
        string $paymentMethodTitle,
        MollieObject $mollieObject
        ...
        if (!$payment->isAuthorized()) {
            $this->logger->debug(
                __METHOD__ . ' order at Mollie not authorized, so no processing for order ' . $orderId
            );
            return;
        }
        ...
        $this->logger->debug(
            __METHOD__ . ' processing order status update via Mollie plugin fully completed for order ' . $orderId
        );
     ...
     * @param MollieObject $mollieObject
     */
    public function onWebhookCompleted(
        WC_Order $order,
        $payment,
        string $paymentMethodTitle,
        MollieObject $mollieObject
        ...
        if (!$payment->isCompleted()) {
            $this->logger->debug(
                __METHOD__ . ' order at Mollie not completed, so no further processing for order ' . $orderId
            );
            return;
        }
     ...
     * @param MollieObject $mollieObject
     */
    public function onWebhookCanceled(
        WC_Order $order,
        $payment,
        string $paymentMethodTitle,
        MollieObject $mollieObject
        ...
        if ($mollieObject->isFinalOrderStatus($order)) {
            $this->logger->debug(
                __METHOD__ . " called for payment {$orderId} has final status. Nothing to be done"
            );
            return;
        }
        ...
        $newOrderStatus = apply_filters(
            $this->pluginId . '_order_status_cancelled_' . $filterSuffix,
            $newOrderStatus
        );
     ...
     * @param MollieObject $mollieObject
     */
    public function onWebhookFailed(
        WC_Order $order,
        $payment,
        string $paymentMethodTitle,
        MollieObject $mollieObject
        ...
        $mollieObject->failedSubscriptionProcess(
            $orderId,
            $gateway,
            $order,
            $newOrderStatus,
     ...
     * @param MollieObject $mollieObject
     */
    public function onWebhookExpired(
        WC_Order $order,
        $payment,
        string $paymentMethodTitle,
        MollieObject $mollieObject
        ...
        if ($alreadyPaid) {
            $this->logger->log(
                ...
                . ', not processed because the order is already paid.'
            );
            return;
        }
        ...
        $filterSuffix = $mollieObject instanceof MollieOrder ? $payment->method : $gateway->id;
        $newOrderStatus = apply_filters(
            $this->pluginId . '_order_status_expired_' . $filterSuffix,
            $newOrderStatus
        );
     ...
     * @param int $orderId
     */
    private function handleOrderPaidSpecifics(
        WC_Order $order,
        $payment,
     ...
     * @param int $orderId
     */
    private function handlePaymentPaidSpecifics(
        WC_Order $order,
        $payment,
    ...
    {
        return $payment->id . ($payment->mode === 'test'
