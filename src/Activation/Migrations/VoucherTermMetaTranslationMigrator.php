 ...
 */
class VoucherTermMetaTranslationMigrator implements \Mollie\WooCommerce\Activation\Migrations\MigratorInterface
{
    ...
    {
        global $wpdb;
        $termIds = $wpdb->get_col($wpdb->prepare("SELECT DISTINCT term_id FROM {$wpdb->termmeta}\n                 WHERE meta_key = %s AND meta_value <> ''", Voucher::MOLLIE_VOUCHER_CATEGORY_OPTION));
        foreach ($termIds as $termId) {
            $termId = (int) $termId;
            $value = get_term_meta($termId, Voucher::MOLLIE_VOUCHER_CATEGORY_OPTION, \true);
            if (!$value) {
                continue;
            }
            ...
            foreach ($this->translatedTermIds($termId) as $translatedTermId) {
                update_term_meta($translatedTermId, Voucher::MOLLIE_VOUCHER_CATEGORY_TRANSLATION_OPTION, $value);
            }
        }
    }
    /**
