...
export const createPaymentComponent = (item, commonProps) => {
    ...
    switch (item.name) {
        ...
        case 'mollie_wc_gateway_billie':
            const BillieFieldsWithStore = withMollieStore(PaymentFieldsComponent);
            return (
        ...
        case 'mollie_wc_gateway_in3':
            const In3FieldsWithStore = withMollieStore(PaymentFieldsComponent);
            return (
                <In3FieldsWithStore
                    {...commonProps}
                    fieldConfig={{
                        ...
                        birthdateLabel: item.birthdatePlaceholder || "Birthdate"
                    }}
                />
            );

        case 'mollie_wc_gateway_riverty':
            const RivertyFieldsWithStore = withMollieStore(PaymentFieldsComponent);
            return (
