...
class OrderItemsRefunder
{
     ...
     */
    public function __construct(\Mollie\WooCommerce\Gateway\Refund\RefundLineItemsBuilder $refundLineItemsBuilder, Data $dataHelper, OrderEndpoint $ordersApiClient)
    {
    ...
    public function refund(WC_Order $order, array $items, Order $remotePaymentObject, string $refundReason): bool
    {
        ...
        $lineItems = $this->refundLineItemsBuilder->buildLineItems($toRefundRemoteItems, $toRefundItems, $this->dataHelper->getOrderCurrency($order), $refundReason);
    ...
    private function normalizedWooCommerceItemsList(array $items): array
    {
        ...
        foreach ($items as $item) {
            $toRefundItemId = $item->get_meta('_refunded_item_id', \true);
            if (!$toRefundItemId) {
                throw new UnexpectedValueException(esc_html__('One of the WooCommerce order items does not have the refund item ID meta value associated to Mollie Order item.', 'mollie-payments-for-woocommerce'));
            }
    ...
    private function normalizedRemoteItems(array $remoteItems): array
    {
        ...
        foreach ($remoteItems as $remoteItem) {
            $orderItemId = $remoteItem->metadata !== null && property_exists($remoteItem->metadata, 'order_item_id') && $remoteItem->metadata->order_item_id !== null ? $remoteItem->metadata->order_item_id : 0;
            if (!$orderItemId) {
                throw new UnexpectedValueException(sprintf(
                    /* translators: Placeholder 1: item id. */
                    esc_html__('Impossible to retrieve the order item ID related to the remote item: %1$s. Try to do a refund by amount.', 'mollie-payments-for-woocommerce'),
