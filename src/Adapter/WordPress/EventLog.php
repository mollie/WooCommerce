...
final class EventLog
{
     ...
     */
    private const ALLOWED_FIELDS = ['cid', 'order', 'session', 'mollie_id', 'wallet', 'surface', 'mode', 'reason', 'kind', 'status', 'amount', 'currency', 'ms'];
    private string $correlationId;
    public function __construct(private LoggerInterface $logger, private ?LoggerInterface $problems = null)
    {
    ...
    private function context(string $event, array $fields): array
    {
        ...
        foreach ($fields as $name => $value) {
            ...
            if ($name === 'cid') {
                continue;
                // The correlation id is the log's own.
            }
            if (in_array($name, self::ALLOWED_FIELDS, \true) && is_scalar($value)) {
                $context[$name] = $value;
                continue;
            }
    ...
    private function reportDropped(string $event, string $field): void
    {
        ...
        if (defined('WP_DEBUG') && \WP_DEBUG) {
            ...
            error_log(sprintf('Mollie EventLog dropped the field "%s" of event "%s": not in docs/architecture/events.md.', preg_replace('/[^A-Za-z0-9_.\-]/', '', $field), preg_replace('/[^A-Za-z0-9_.\-]/', '', $event)));
        }
    }
}
