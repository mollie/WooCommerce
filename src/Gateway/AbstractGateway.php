...
abstract class AbstractGateway extends WC_Payment_Gateway
{
	...
	public function process_payment( $order_id ) {
		...
		if ( ! $order ) {
			Plugin::debug( $this->id . ': Could not process payment, order ' . $order_id . ' not found.' );
    ...
    public function onWebhookAction ()
    {
        ...
        Plugin::debug($this->id . ": MollieSettingsPage payment object {$payment->id} (" . $payment->mode . ") webhook call for order {$order->get_id()}.", true);
    ...
    protected function processChargebacks(WC_Order $order, $payment)
    {
        ...
        try {
            ...
            // Overwrite gateway-wide
            $newOrderStatus = apply_filters( Plugin::PLUGIN_ID . "_order_status_on_hold_{$this->id}", $newOrderStatus );
	...
	protected function orderNeedsPayment( WC_Order $order ) {
		...
		if ( $this->isOrderPaidByOtherGateway( $order ) ) {
			Plugin::debug( __METHOD__ . ' ' . $this->id . ': Order ' . $order_id . ' orderNeedsPayment check: no, previously processed by other (non-Mollie) gateway.', true );
		...
		if ( ! $this->isOrderPaidAndProcessed( $order ) ) {
			Plugin::debug( __METHOD__ . ' ' . $this->id . ': Order ' . $order_id . ' orderNeedsPayment check: yes, order not previously processed by Mollie gateway.', true );

			return true;
		}

		if ( $order->needs_payment() ) {
			Plugin::debug( __METHOD__ . ' ' . $this->id . ': Order ' . $order_id . ' orderNeedsPayment check: yes, WooCommerce thinks order needs payment.', true );
		...
		if ( $this->getInitialOrderStatus() === self::STATUS_ON_HOLD && $order->has_status( self::STATUS_ON_HOLD ) ) {
			Plugin::debug( __METHOD__ . ' ' . $this->id . ': Order ' . $order_id . ' orderNeedsPayment check: yes, has status On-Hold. ', true );
    ...
    protected function updateStateRefund(
        ...
        // Overwrite gateway-wide
        $newOrderStatus = apply_filters(
            Plugin::PLUGIN_ID . $refundType . $this->id,
            $newOrderStatus
        );
        // New order status
