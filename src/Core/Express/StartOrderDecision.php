...
final class StartOrderDecision
{
    ...
    public static function decide(ExpressOrderFacts $facts, CartFacts $cart, int $now): Refuse|string
    {
        ...
        if ($facts->fingerprint() !== PricingFingerprint::of($cart)) {
    ...
    public static function stamps(ExpressOrderFacts $facts, string $mode, string $gatewayId, string $wallet): array
    {
        return [
            ...
            Effect::setMeta('_mollie_express_ref', (string) $facts->expressRef()),
            Effect::setMeta('_mollie_express_session_id', (string) $facts->sessionId()),
            Effect::setMeta('_mollie_express_expires_at', (string) $facts->expiresAt()),
