...
class AsYouTypeFormatter
{
    ...
    private function getAvailableFormats($leadingDigits)
    {
        ...
        foreach ($formatList as $format) {
            ...
            // prefix.
            if ($this->extractedNationalPrefix !== '' && PhoneNumberUtil::formattingRuleHasFirstGroupOnly($format->getNationalPrefixFormattingRule()) && !$format->getNationalPrefixOptionalWhenFormatting() && !$format->hasDomesticCarrierCodeFormattingRule()) {
    ...
    private function isDigitOrLeadingPlusSign($nextChar)
    {
        ...
        return \preg_match('/' . PhoneNumberUtil::DIGITS . '/' . PhoneNumberUtil::REGEX_FLAGS, $nextChar) || \mb_strlen($this->accruedInput) === 1 && $plusCharsMatcher->matches();
    }
    /**
    ...
    private function appendNationalNumber($nationalNumber)
    {
        ...
        if ($this->shouldAddSpaceAfterNationalPrefix && $prefixBeforeNationalNumberLength > 0 && \mb_substr($this->prefixBeforeNationalNumber, $prefixBeforeNationalNumberLength - 1, 1) != self::$separatorBeforeNationalNumber) {
