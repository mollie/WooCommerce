...
function gatewayContainer( container ) {
	return container
		? container.querySelector( SELECTOR_MOLLIE_GATEWAY_CONTAINER )
		: null;
}
...
function noticeContainer( container ) {
	return container
		? container.querySelector( SELECTOR_MOLLIE_NOTICE_CONTAINER )
		: null;
}

function componentsContainerFromWithin( container ) {
	return container
		? container.querySelector( SELECTOR_MOLLIE_COMPONENTS_CONTAINER )
		: null;
}
...
function scrollToNotice( jQuery ) {
	let scrollToElement = noticeContainer( document );
...
function isGatewaySelected( gateway ) {
	...
	const gatewayInput = gatewayContainer
		? gatewayContainer.querySelector(
				`#payment_method_mollie_wc_gateway_${ gateway }`
		  )
		: null;
...
function componentElementByNameFromWithin( name, container ) {
	return container
		? container.querySelector( `.mollie-component--${ name }` )
		: null;
}
...
function mountComponent(
	...
	! mollieComponentsMap.has( componentName ) &&
 ...
 * @param root0
 * @param root0.options
 * @param root0.merchantProfileId
 * @param root0.componentsSettings
 * @param root0.componentsAttributes
 * @param root0.enabledGateways
 * @param root0.messages
 * @param mollieComponentsMap
 */
...
( function ( { _, Mollie, mollieComponentsSettings, jQuery } ) {
	...
	function checkInit() {
		return function () {
			const copySettings = JSON.parse(
				JSON.stringify( mollieComponentsSettings )
			);
			mollieComponentsSettings.enabledGateways.forEach(
				function ( gateway, index ) {
					const gatewayContainer = containerForGateway(
						gateway,
						document
					);
					if ( ! gatewayContainer ) {
