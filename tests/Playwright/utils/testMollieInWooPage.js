...
export const wooOrderPaidPage = async (page, mollieOrder, totalAmount, testedGateway) => {
    page.reload();
    // Check order number
    await expect(page.getByText(/Order number: .+/)).toContainText(mollieOrder);
    // Check total amount in order
    totalAmount = totalAmount.split("\n")[0];
    await expect(page.getByText(/Total: .+/).first()).toContainText(totalAmount);
    ...
    await expect(page.getByText(methodName).first()).toBeVisible();
}
...
export const wooOrderDetailsPage = async (page, mollieOrder, testedGateway, status, notice, baseURL) => {
    // Check order is in status processing
    const orderStatus = await fetchOrderStatus(mollieOrder, baseURL);
    await expect(orderStatus.toUpperCase()).toBe(status.toUpperCase());

    // Check order notes has correct text
    const orderNotesArray = await fetchOrderNotes(mollieOrder, baseURL);
