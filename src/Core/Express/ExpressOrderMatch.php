...
final class ExpressOrderMatch
{
    ...
    public static function decide(PaymentSnapshot $payment, ?ExpressOrderFacts $order): Admit|Refuse
    {
        ...
        if ($order->createdVia() !== \Mollie\WooCommerce\Core\Express\StartOrderDecision::CREATED_VIA) {
    ...
    {
        $total = $order->total();
        return $total !== null && $total->currency() === $payment->amount()->currency() && $total->equals($payment->amount());
    }
