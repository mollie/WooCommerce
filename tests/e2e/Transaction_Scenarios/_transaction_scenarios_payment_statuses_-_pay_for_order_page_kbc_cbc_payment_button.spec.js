...
test.describe('_Transaction scenarios_Payment statuses - Pay for order page - KBC_CBC Payment Button', () => {
    ...
    testData.forEach(({ testId, mollieStatus, wooStatus, notice, action }) => {
        test(`[TestId-${testId}] Validate the submission of an order with KBC_CBC as payment method and payment mark as "${mollieStatus} on pay for order page"`, async ({ page, products, context }) => {
