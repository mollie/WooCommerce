...
test.describe('_Transaction scenarios_Payment statuses - Pay for order page - paysafecard', () => {
    ...
    testData.forEach(({ testId, mollieStatus, wooStatus, notice, action }) => {
        test(`[${testId}] Validate the submission of an order with paysafecard as payment method and payment mark as "${mollieStatus} on pay for order page"`, async ({ page, products, context }) => {
