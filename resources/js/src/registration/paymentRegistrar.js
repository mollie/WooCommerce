 ...
 * @returns {boolean} True if the payment method can be registered, false otherwise
 * @example
 * const paymentMethod = {
...
export const canRegisterPaymentMethod = (item) => {
    try {
        ...
        if (ApplePayUtils.isApplePayMethod(item) && config.requiresAppleSession) {
            return ApplePayUtils.canRegisterApplePay();
        }

        return true;
    } catch (error) {
        console.warn(`Mollie: Cannot register payment method ${item?.name}:`, error.message);
        return false;
    }
};
 ...
 * @returns {Object} The registered payment method object from WooCommerce
 ...
 * }
 */
export const registerSinglePaymentMethod = (item, context, type = PAYMENT_TYPES.REGULAR) => {
    try {
        ...
        console.error(`Mollie: Failed to register ${type} ${item.name}:`, error.message);
        throw error;
    }
};
 ...
 * @param {PaymentItem} item - The payment method item to register
 * @param {PaymentContext} context - The registration context containing WC blocks registry and dependencies
 * @returns {Array<RegistrationResult>} Array of registration results for each variant attempted
 * @example
...
export const registerPaymentMethod = (item, context) => {
    ...
    try {
        ...
        if (config.express) {
            ...
            results.push({ success: true, name: `${item.name}_express`, type: 'express' });
        }
 ...
 * @param {PaymentContext} context - The registration context containing WC blocks registry and dependencies
 * @returns {Array<RegistrationResult>} Array of all registration results (successful and failed)
 * @example
 * const gatewayData = [
 *     { name: 'mollie_wc_gateway_ideal', title: 'iDEAL' },
...
export const registerAllPaymentMethods = (gatewayData, context) => {
    ...
    console.log(`Mollie: Registered ${successful}/${allResults.length} payment method variants`);
    if (failed.length > 0) {
