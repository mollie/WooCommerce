...
test.describe('_Transaction scenarios_Payment statuses Checkout - KBC_CBC Payment Button', () => {
    ...
    testData.forEach(({ testId, mollieStatus, wooStatus, notice, action }) => {
        test(`[TestId-${testId}] Validate the submission of an order with KBC as payment method and payment mark as "${mollieStatus}"`, async ({ page, products, context }) => {
