...
final class StartExpressOrder
{
    ...
    public function start(): \Mollie\WooCommerce\Workflow\ExpressOrderResult
    {
        ...
        try {
            return $this->lock->withLock($ref, function () use ($facts): \Mollie\WooCommerce\Workflow\ExpressOrderResult {
    ...
    private function startLocked(ExpressOrderFacts $facts): \Mollie\WooCommerce\Workflow\ExpressOrderResult
    {
    ...
    private function create(ExpressOrderFacts $facts, CartFacts $cart): \Mollie\WooCommerce\Workflow\ExpressOrderResult
    {
        ...
        $this->log->info('express.order.created', ['order' => $order->get_id(), 'session' => $facts->sessionId(), 'wallet' => $wallet, 'amount' => $total->toDecimal(), 'currency' => $total->currency()]);
        return $this->answer();
    }
    /**
     ...
     */
    private function answer(): \Mollie\WooCommerce\Workflow\ExpressOrderResult
    {
        $details = $this->orderFacts->shopperDetails();
        return \Mollie\WooCommerce\Workflow\ExpressOrderResult::ok(AddressMapping::toMollie($details['billing']), AddressMapping::toMollie($details['shipping']));
    }
    /**
    ...
    private function sameAmount(WC_Order $order, Money $total): bool
    {
        ...
        return number_format((float) $order->get_total('edit'), $decimals, '.', '') === $expected && $order->get_currency() === $total->currency();
    }
    private function refuse(ExpressOrderFacts $facts, string $code, int $httpStatus, ?string $reason = null): \Mollie\WooCommerce\Workflow\ExpressOrderResult
    {
