...
final class Money
{
    ...
    private function __construct(private int $minorUnits, private string $currency)
    {
    }
    ...
    public static function fromDecimal(string $value, string $currency): self
    {
        ...
        if (preg_match($pattern, $value, $matches) !== 1) {
            throw new InvalidArgumentException(sprintf('"%s" is not a valid %s amount with %d decimals.', $value, $currency, $decimals));
        }
    ...
    private function assertSameCurrency(self $other): void
    {
        if ($this->currency !== $other->currency) {
            throw new InvalidArgumentException(sprintf('Amounts in %s and %s cannot be combined.', $this->currency, $other->currency));
        }
    }
    private static function decimalsOf(string $currency): int
    {
        return in_array($currency, self::ZERO_DECIMAL_CURRENCIES, \true) ? 0 : 2;
    }
