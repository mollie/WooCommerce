...
test.describe('_Transaction scenarios_Payment statuses Checkout - SEPA Bank Transfer', () => {
    ...
    testData.forEach(({ testId, mollieStatus, wooStatus, notice, action }) => {
        test(`[${testId}] Validate the submission of an order with SEPA Bank Transfer as payment method and payment mark as "${mollieStatus}"`, async ({ page, products, context }) => {
