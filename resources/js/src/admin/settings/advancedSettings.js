	...
	jQuery( document ).ready( function ( $ ) {
		$( '.mollie-settings-advanced-payment-desc-label' )
			...
			.on( 'mousedown', function ( e ) {
				...
				const $this = $( this );
				$( window ).on(
					'mouseup.molliesettings drag.molliesettings blur.molliesettings',
					function ( e ) {
						$this.off( '.molliesettings' );
...
function toggleManualCaptureFields( onHoldSelect ) {
	...
	const captureStatusChangeField = jQuery(
	...
	const captureStatusChangeFieldParent =
		captureStatusChangeField.closest( 'tr' );
